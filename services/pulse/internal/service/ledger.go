...
func (s *LedgerService) Append(ctx context.Context, requested ledger.Entry) (ledger.Entry, error) {
	...
	err := s.unit.Do(ctx, func(repos ports.Repositories) error {
		...
		// Check before locking the account for the common replay path. The
		// second check after locking closes the race with another writer.
		existing, err := repos.Ledger.FindByIdempotency(ctx, requested.Operation, requested.IdempotencyKey)
		if err != nil && !errors.Is(err, ports.ErrNotFound) {
			return err
		}
		if existing != nil {
			return reuseOrConflict(requested, *existing, &result)
		}
...
func reuseOrConflict(requested, existing ledger.Entry, result *ledger.Entry) error {
