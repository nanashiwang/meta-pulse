...
Meta Pulse 负责 Usage Event、贡献值、脉冲券、长期经济规则快照、Reward、Budget、Experiment、增长分析和奖励结算状态。数据库中的历史快照仅用于审计和保证旧券规则不变，不再作为用户可见的周期概念。

管理员在 **Pulse 配置 → 设置兑换比例** 配置贡献倍率、产券门槛、额度资格天数（默认 30 天）、奖项权重和预算。规则长期生效，用户是否可参与只看社区账号活跃状态和真实付费贡献；新券满设定天数后仅抽经验，已有券保留获得时的规则，剩余贡献持续累计。详见 [管理配置说明](docs/METAR_ADMIN_SETTINGS.md)。
...
METAR 管理员可在 `/admin/pulse` 配置额度换算、抽奖开关、影子模式与各用途的 HMAC 密钥，并通过「设置兑换比例」保存长期生效的贡献倍率、每券贡献度和奖池规则。已保存的规则快照只用于审计及保护已有券，不在产品界面展示周期切换。首次在 Answer 插件后台完成管理通道配对，之后无需为这些参数反复修改 `.env` 或重启；密钥按 API/Worker 分角色加密，保存有版本校验、幂等与审计。启用、备份和恢复步骤见 [METAR 管理员配置](docs/METAR_ADMIN_SETTINGS.md)。
