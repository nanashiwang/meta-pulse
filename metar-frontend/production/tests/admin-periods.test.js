...
function harness() {
  ...
  const nodes = new Map();
  const form={inputs:{key:'next-1',starts_at:'2030-01-01T00:00',multiplier:'1.2345',threshold:'123.456',reward_budget:'100',reason:'Reviewed rates',confirm:'on'},
    querySelector:s=>{if(!nodes.has(s))nodes.set(s,{});return nodes.get(s);},
    querySelectorAll:()=>[{querySelector:s=>({value:s.includes('prize_key')?'reward':s.includes('prize_amount')?'10':'1'})}],reset(){}};
...
test('form sends complete frozen pool with exact integers and explicit UTC+8 date',()=>{
 ...
 assert.equal(p.starts_at,'2030-01-01T00:00:00+08:00');assert.equal(p.reward_budget,100);
...
test('lost response survives refresh and retries identical payload and key; no duplicate submission',async()=>{
 ...
 h.context.fetch=async(url,options)=>{
  ...
  return {ok:true,status:200,json:async()=>({period_id:1,period_key:'next-1',status:'active'})};
 };
