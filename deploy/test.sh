...
[[ "$(grep -c 'proxy_set_header X-Pulse-Signature "";' "$ROOT/deploy/nginx/meta-pulse.conf")" -eq 9 ]] || { echo '所有 9 个社区代理入口必须清除 Pulse 签名' >&2; exit 1; }
[[ "$(grep -c 'proxy_set_header New-Api-User "";' "$ROOT/deploy/nginx/meta-pulse.conf")" -eq 9 ]] || { echo '所有 9 个社区代理入口必须清除外部用户 ID' >&2; exit 1; }
