...
text = (root/'deploy/meta-pulse.env.example').read_text()
# Trust-role secret reuse is rejected in production. Give every role a stable,
# distinct public test value before replacing the non-secret DB placeholders.
for index, key in enumerate((
    'PULSE_SERVICE_HMAC_SECRET', 'PULSE_FORUM_HMAC_SECRET',
    'PULSE_USER_BFF_HMAC_SECRET', 'PULSE_ADMIN_HMAC_SECRET',
    'PULSE_REWARD_RANDOM_SECRET',
), start=1):
    text = text.replace(f'{key}=replace-me', f'{key}=test-only-{index}-'+'x'*48, 1)
text = text.replace('replace-me', 'test-only-'+'x'*48)
...
forum_env = {k:str(v) for k,v in services['forum']['environment'].items() if v is not None}
assert forum_env.get('FORUM_BINDING_GUARD_DSN'), 'forum binding guard DSN is missing'
assert forum_env['FORUM_BINDING_GUARD_DSN'].startswith('forum:'), 'forum guard must use the dedicated forum account'
for forbidden in ('PULSE_DB_DSN','NEWAPI_LOG_DSN','PULSE_SERVICE_HMAC_SECRET','PULSE_FORUM_HMAC_SECRET','PULSE_USER_BFF_HMAC_SECRET','PULSE_ADMIN_HMAC_SECRET'):
    assert not forum_env.get(forbidden), f'forum gained forbidden credential {forbidden}'
for service_name in ('pulse-api','pulse-worker'):
