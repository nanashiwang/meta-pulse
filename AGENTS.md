...
对外产品入口可以“社区为主、Pulse 权益为辅”，但不得改变上述经济边界。
...
18. 论坛允许 Answer 本地独立注册；社区身份来自 Answer，API/资金身份来自 new-api。
19. Answer 与 new-api 的可选绑定必须一对一，禁止静默换绑、身份转移和普通解绑。
20. 未绑定用户和绑定前发布的内容不得产生 Pulse 内容奖励候选或奖励。
21. Pulse 故障不得阻断论坛本地登录或浏览。
22. 论坛登录回调是浏览器重定向，所有参数在验签通过前一律视为攻击者可控。
23. new-api 未证明邮箱已验证时，Connector 不得把该邮箱交给 Answer 自动绑定。
24. 论坛本地封禁、密码、资料和治理角色不得被 new-api/Pulse UserCenter 覆盖。
