...
✅ METAR 配置页     Answer 管理员鉴权、角色加密配置、CAS/幂等/审计和 API/Worker 热读取
```
...
- [x] Outbox 指数退避；`dead` 表示重试耗尽且仍可 Query/Reconcile，`conflict` 表示不可自动收敛的终态完整性冲突；`reward-retry --grant-id <grant_id>` 只允许精确恢复 `dead`，先 Query 原 `source_ref`，仅 `not_found` 原子领取一次额外尝试且保留累计 attempts；
...
- [x] M0-M6 主线代码、迁移和回归测试；隔离 MySQL 集成测试已纳入 CI，生产 Compose 联调仍见第 8 节；
...
以下是必须由真实外部系统、账号、流量和密钥完成的上线验收；本仓库的加固、回归和 CI 不能替代真实结果：
...
- [x] Action 跨周期/关闭后重放使用稳定身份；换 key 不能让同一 action 再发奖，同 key 改 payload 返回 conflict；兼容旧版周期范围记录，歧义结果交人工对账。
- [x] API/Worker/Tool 按角色检查生产密钥；Worker 不持有 BFF/Admin 密钥；新增不连接数据库的 `config-check --role` 和真实 Compose 渲染回归。
- [x] Worker 每项任务独立循环、独立超时、单任务不重叠；慢数据源不再使后续结算/对账继承过期 context。
...
- 新增社区管理入口、管理员实时角色/激活状态复核，以及显式管理密钥配对；无需 new-api 绑定。
- 公共运行参数与分角色密文落入 Pulse DB，配置版本、幂等响应和审计同事务；API/Worker 私钥分卷，发奖密钥只由 Worker 解密。
- API 新请求和 Worker 新任务使用最新配置，失败不回退旧开关/密钥；随机种子和 Period 经济参数不开放修改，结算目标登记后锁定。
- 新增迁移 `00012`、私钥卷备份、并发/重放/权限/密文隔离/丢响应恢复回归。真实服务器升级、首次配对、私钥恢复演练及实际到账仍需部署验收。

## 15. 网页经济周期配置

- 管理员可在 Pulse 配置页设置新周期贡献倍率与产券门槛，并一次创建完整奖池；已启用周期只读。
- Answer 管理身份复核、固定 BFF 路由、同源写保护、整数校验、事务幂等与审计复用既有安全边界。
- MySQL 回归覆盖 100 次并发重放、不同请求创建重叠周期的冲突以及刷新后读取保存值；网页覆盖精确小数、丢响应恢复、旧后端降级。生产更新仍需按发布流程部署，不自动变更真实周期或发奖开关。

## 社区成长扩展

- 社区 EXP 独立账本、五倍等级门槛、签到、观察期内容事件、来源撤销、规则版本、成长页与管理入口。
- 真实 MySQL 并发/幂等/上限/撤销回归纳入 CI；本地真实 Answer 验收与线上部署分别记录。
- Pulse 经验奖项需独立预算与可靠交付，不能将 EXP 发送至 new-api 额度接收器，也不能修改已生效奖池概率。
