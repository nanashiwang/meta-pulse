...
METAR 社区权益页通过按需加载的轻量 Canvas 展示“脉冲核心”演出：微光汇聚、晶体蓄能、裂变与奖励卡显现。视觉时间线等待已认证 Action 响应后才进入揭晓；跳过、减少动态效果和页面切换不创建额外 Action。服务端 Grant 仍是唯一结果来源，浏览器不随机选奖、不乐观扣券，也不把揭晓等同于已到账。沿用既有按用户隔离的操作编号恢复语义；页面刷新后的结果从原 action 查询恢复，券数和结算状态从服务端重新读取。动画结束后局部刷新数据，故障不抹除已确认的奖励显示。
...
后续额度总量模式：新持续规则支持 `quota_budget_unlimited=true`，对应 `pulse_reward_budget.unlimited=true`、`budget_type=loyalty`、`hard_cap=0`。零 cap 只有和显式 unlimited 标记同时出现才代表不限额度总量；旧数据默认 false，不能因 cap 为零放行。概率控制每券数学期望，不保证任何时间窗口内的实际支出上限。经验预算仍有限，已启用规则不原地改写。

迁移 00015 扩展预算约束和冻结触发器。不限额度奖池继续锁定预算行，记录预留、结算与撤销；应用和数据库均检查非负值及 int64 累计边界，继续保留 Ticket、Grant、Outbox 的事务和幂等语义。不通过删除预算行、伪造巨大 cap 或绕过数据库记账实现不限额度。保存模式进入审计与幂等指纹，仅支持持续规则；原有限预算和经验池依然执行硬上限。若已有不限额度数据，迁移 Down 拒绝执行，不能静默转换为有限额度。

网页默认勾选“不限制额度奖励总量”，允许取消勾选并设置有限额度预算；旧规则列表回显各自模式。期望额度按 `Σ(额度奖项数量×权重) / Σ(全部奖项权重)` 使用整数有理数计算，经验奖项贡献零额度但计入分母。显示原始比例和小数近似，供运营调参，不作为账本金额。后端通过 `unlimited_quota_supported` 声明能力；旧后端禁用新编辑器。

网页不再要求运营人员设置周期。`pulse_period` 同时承载不可变的规则快照：`continuous=true`、`quota_validity_days`（1—3650，网页默认 30），开始时间由服务器生成，结束时间使用 9999-01-01 哨兵值，不执行 Period Close。历史十天活动继续原有状态机，既有经济规则不改写。

新 Usage 按源事件时间选择已启用的最近持续快照，同一秒按 ID 较大者确定。已记账事件重放仍恢复原记录。保存新规则受全局创建锁和 `expected_period_id` 乐观检查保护；同请求恢复首次结果，新的服务器时间不进入重试指纹。时区为 Asia/Shanghai，数据库 DATETIME 写入与读取使用一致的本地时间。

持续规则的未成券贡献跨快照累计，按“持续规则贡献账本净额减去各快照铸券量乘以对应冻结门槛”重建，以任意精度中间运算检查 int64 溢出。当前实现对用户历史账本作锁定当前读，避免 Repeatable Read 下看到旧余额；读取成本随用户历史增长，后续缓存必须可从该公式重建，不能替代账本事实。

迁移 `00014` 新增 `pulse_ticket_lot` 发行快照、`pulse_ticket_allocation` 追加式消费分配和 `pulse_ticket_user_lock` 用户事务锁。Usage 同一事务内写入贡献、铸券账本和发行批次；获得时间取实际入账时刻，期限为获得时间加 N×24 小时，均以 UTC 秒存储。铸券账本 metadata 同时记录获得时间与额度截止时间；消费账本 metadata 指向铸券 entry。批次剩余数量等于发行数减去关联的消费分配，可对照账本重建；金额事实仍以 Ledger 为准，批次不能独立增发券。数据库禁止更改发行身份、时间和期限，禁止更新或删除消费分配。

Action 在用户事务锁内优先消费仍有效的历史周期券，再按获得时间、批次 ID 消费持续券。每次只消费一张。已有券使用发行时规则快照，不被新配置更改；持续券到期不销毁，而是在 `now >= quota_expires_at` 后仅从原 `community_exp` 子奖池按原权重归一化抽取。每个持续规则必须有经验奖项。所有抽取使用原版本 HMAC 和既有幂等 Grant，中奖不减少权重，无库存式不放回机制。

每个快照拥有独立的额度记账和经验预算；额度可按上文配置为不限总量，有限额度模式仍分配独立硬预算，不是为旧券补预算。任一适用的有限奖池余额不足以覆盖最大单次奖励时拒绝新抽取，不扣券、不重随机、不调整概率。过期券只检查经验预算；幂等恢复优先于时效和配置检查。已有 Outbox、Benefit source_ref、经验交付和对账语义不变。

公开规则使用验签身份选择下一张券，与 Action 共用选择函数，展示该券额度截止时间及适用奖项概率；summary 合计仍可消费的历史与持续券。网页明确披露到期仅抽经验。管理员入口仍为 `/admin/pulse`，GET 返回 `continuous_supported`；旧后端不声明支持时禁用新表单。配置仅限 Answer 实时复核的管理员，无浏览器自报身份，且不自动启用抽奖或发奖。

历史活动和旧 CLI 保留固定 10 天周期；新网页规则采用上文“持续规则与券龄”方案，不需要手动周期。历史状态机：
...
11. 有限预算的 Reward 不得突破 Hard Budget；显式不限总量的持续额度奖池仍须保证整数记账和完整审计。
...
公开内容与身份查询并行，受保护页面等待身份快照完成；身份失败保留公开浏览并对私人入口失败关闭。公开问题列表、标签、搜索允许当前文档内存中最多 24 项、15 秒的短暂复用与同请求合并，按 Answer token 和语言隔离；身份变化、主动刷新、主动搜索和 bfcache 恢复清理，失败不缓存。身份、绑定、权限、个人信息、通知、成长、Pulse 和管理接口不进入此缓存，服务状态始终执行实时查询。缓存只是 Answer 响应的短期浏览加速，不构成新的内容或财务事实源。站内切页复用导航，滚动位置仅写入本页面的 history state；迟到响应不替换新页面，也不重置其滚动。
...
### 管理员网页创建经济周期（v0.2.13—v0.2.16 历史方案）
